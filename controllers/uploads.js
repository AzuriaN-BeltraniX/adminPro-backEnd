...
// Controlador para subir archivos, asignándolo a un usuario:
const fileUpload = (req, res = response) => {
    ...
    const tiposValidos = ['hospitals', 'doctors', 'users']; // Define las colecciones válidas...
    // ...Si no se incluye una colección válida, entonces...
    if (!tiposValidos.includes(tipo)) {
        return res.status(406).json({ // Muestra estado de la petición
            ok: false,
            msg: `El termino "${termino}", no es válido o no existe. Los términos válidos son: 'hospitals', 'doctors', 'users'`
